...
async function create(req, res) {
  ...
  const id = uuidv4();
  await pool.query(
    'INSERT INTO daily_tasks (id, title, content, task_date, created_by) VALUES (?, ?, ?, ?, ?)',
    [id, title, content, task_date, req.user.id]
  );
...
async function update(req, res) {
  const { title, content, task_date } = req.body;
  await pool.query(
    'UPDATE daily_tasks SET title = ?, content = ?, task_date = ? WHERE id = ?',
    [title, content, task_date, req.params.id]
  );
  const [rows] = await pool.query('SELECT * FROM daily_tasks WHERE id = ?', [req.params.id]);
  res.json(rows[0]);
}

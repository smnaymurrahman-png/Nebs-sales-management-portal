...
async function login(req, res) {
  ...
  const [rows] = await pool.query('SELECT * FROM users WHERE work_email = ?', [work_email]);
...
async function me(req, res) {
  const [rows] = await pool.query(
    'SELECT id, full_name, work_email, designation, role, created_at FROM users WHERE id = ?',
    [req.user.id]
  );
...
async function updateProfile(req, res) {
  ...
  await pool.query('UPDATE users SET full_name = ?, designation = ? WHERE id = ?', [full_name, designation, req.user.id]);
  const [rows] = await pool.query('SELECT id, full_name, work_email, designation, role FROM users WHERE id = ?', [req.user.id]);
  res.json(rows[0]);
}

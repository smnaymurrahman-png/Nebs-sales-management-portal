...
async function update(req, res) {
  ...
  const { rows } = await pool.query(
    `UPDATE clients SET client_name=$1, whatsapp_number=$2, whatsapp_link=$3, data_requirements=$4,
     ...
     WHERE id=$14 AND added_by=$15 RETURNING *`,
    [client_name, whatsapp_number, whatsapp_link, data_requirements, type,
     ...
     client_type || null, data_type || null, last_message, last_message_image || null, remarks, req.params.id, req.user.id]
  );
...
async function remove(req, res) {
  const { rowCount } = await pool.query(
    'DELETE FROM clients WHERE id = $1 AND added_by = $2',

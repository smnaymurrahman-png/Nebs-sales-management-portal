...
const VENDOR_TYPE_COLORS: Record<string, string> = {
  ...
  'Dialer provider': 'bg-violet-500/20 text-violet-300',
  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: max }).map((_, i) => (
        <Star key={i} size={11} className={i < filled ? 'text-amber-400 fill-amber-400' : 'text-slate-700'} />
      ))}
    </div>
  );
}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {loadingRatings ? (
            ...
            <p className="text-slate-500 text-sm text-center py-8">No ratings yet. Be the first!</p>
          ) : (
            ratings.map(r => (
              <div key={r.id} className="bg-slate-800 rounded-xl p-3 space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-500">{r.rated_by_name}</span>
                </div>
        ...
        {/* Submit rating */}
        <form onSubmit={submitRating} className="p-5 border-t border-slate-800 space-y-3 shrink-0">
              ...
              return (
                ...
                >
                  <Star
                    ...
                    className={cn(
                      'transition-colors',
                      val <= display ? 'text-amber-400 fill-amber-400' : 'text-slate-700 hover:text-slate-500'
                    )}
                  />
                </button>
              );
          ...
          <div className="flex justify-end">
            <button type="submit" disabled={submitting || !myRating}
              className="flex items-center gap-2 px-4 py-2 text-sm bg-violet-600 hover:bg-violet-700 text-white rounded-xl disabled:opacity-50">
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg animate-slide-up">
        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
          <h2 className="font-semibold text-white">{vendor ? 'Edit Vendor' : 'Add Vendor'}</h2>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Telegram ID</label>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-x-auto">
            <table className="w-full text-sm min-w-[900px]">
              ...
              <tbody className="divide-y divide-slate-800">
                {filtered.length === 0 && (
                  <tr><td colSpan={7} className="py-12 text-center text-slate-500">No vendors found</td></tr>
                )}
                {filtered.map(v => {
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {v.vendor_type && (
                          <span className={cn('text-xs px-2 py-0.5 rounded-full', VENDOR_TYPE_COLORS[v.vendor_type] || 'bg-slate-700 text-slate-300')}>
                            {v.vendor_type}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3 space-y-0.5">
                        {v.phone_number && <p className="text-slate-300 text-xs">{v.phone_number}</p>}
                        {v.whatsapp_link && (
                      ...
                      <td className="px-4 py-3 text-slate-300 text-sm">{v.country || '—'}</td>
                      <td className="px-4 py-3">
                        <button
                          ...
                          {avg !== null ? (
                            <>
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-xs text-slate-500">{v.rating_count} review{Number(v.rating_count) !== 1 ? 's' : ''}</span>
                              </div>
                              <StarDisplay value={avg} />
                            </>
                          ) : (
                            <span className="text-xs text-slate-600 group-hover:text-violet-400 transition-colors">Rate this vendor</span>
                          )}
                        </button>
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-xs">{v.added_by_name}</td>
                      <td className="px-4 py-3">
                        <div className="flex gap-1">

    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-slide-up">
        <div className="p-5 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white">
          <h2 className="font-semibold text-gray-900">{item ? 'Edit Facebook Page ID' : 'Add Facebook Page ID'}</h2>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Page Link</label>
          ...
          {Object.entries(counts).map(([status, count]) => (
            <button key={status} onClick={() => setFilterStatus(filterStatus === status ? '' : status)}
              className={cn('px-3 py-1 rounded-xl text-xs font-medium border transition-colors',
                filterStatus === status ? PAGE_STATUS_COLORS[status] + ' border-current' : 'bg-white border-gray-200 text-gray-500 hover:text-gray-900')}>
              {status} · {count}
            </button>
          ))}
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-x-auto">
            <table className="w-full text-sm min-w-[860px]">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.length === 0 && <tr><td colSpan={8} className="py-12 text-center text-gray-400">No Facebook Page IDs found</td></tr>}
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50/80 transition-colors">
                    <td className="px-4 py-3">
                      ...
                      {item.remarks && <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[200px]">{item.remarks}</p>}
                    </td>
                    <td className="px-4 py-3 text-gray-600 font-mono text-xs">{item.page_id || '—'}</td>
                    <td className="px-4 py-3 text-gray-600">{item.fb_email || '—'}</td>
                    <td className="px-4 py-3">
                      <span className={cn('text-xs px-2 py-0.5 rounded-full', PAGE_STATUS_COLORS[item.page_status] || 'bg-gray-200 text-gray-600')}>
                        {item.page_status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-600">{item.connected_whatsapp || '—'}</td>
                    <td className="px-4 py-3 text-gray-600">{item.page_likes?.toLocaleString() ?? 0}</td>
